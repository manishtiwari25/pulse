...
6. Never touch project-owned content: `docs/context/`, `docs/architecture/`, non-template files in `docs/decisions|features|memory|plans/`, `docs/usage/usage-log.md` rows, or any product code.
7. Update `.template-sync` with the new commit and date. Leave everything uncommitted for the user to review; do not commit or push unless asked.
8. If validation fails, execute the scoped rollback plan and verify the prior state.
9. Report: what was synced, what needs manual review, what was skipped, and whether the repo is now up to date with the template.
